...
- Change Fcitx5 configuration: edit `home/.config/fcitx5/profile` or `home/.config/environment.d/90-fcitx5.conf`; use `config/conf.d/packages.pacman.toml` for the native Arch daemon/engine, `config/config.fcitx-flatpak.toml` for the non-Arch Flatpak daemon/engine/autostart, APT/DNF package fragments for host IM modules, and `config/scripts/sync-fcitx5-themes.sh` plus the `pre-dotfiles` hook for Catppuccin and Mellow themes.
