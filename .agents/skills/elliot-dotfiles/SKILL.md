...
- Keep general Flatpak app/bootstrap configuration in `config/config.flatpak.toml`. The APT graphical Fcitx Flatpak stack is isolated in `config/config.fcitx-flatpak.toml` and selected by the `fcitx-flatpak` environment.
...
- Change Fcitx5 configuration: edit `home/.config/fcitx5/profile` or `home/.config/environment.d/90-fcitx5.conf`; use `config/conf.d/packages.pacman.toml` for Arch, `config/conf.d/distro.fedora.toml` plus `packages.dnf.toml` for Fedora, and `config/config.fcitx-flatpak.toml` for APT graphical systems. Use `config/scripts/sync-fcitx5-themes.sh` plus the `pre-dotfiles` hook for Catppuccin and Mellow themes, and `config/scripts/reload-fcitx5.sh` plus the `post-dotfiles` hook to activate restored configuration.
