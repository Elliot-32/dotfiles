...
- Keep general Flatpak app/bootstrap configuration in `config/config.flatpak.toml`. The non-Arch graphical Fcitx Flatpak stack is isolated in `config/config.fcitx-flatpak.toml` and selected by the `fcitx-flatpak` environment.
...
- Prefer native `mode = "track"` for ordinary user dotfiles. Keep them at the path applications actually read (`home/...` in the setup repository) instead of introducing source-to-target symlinks. Fcitx5 user configuration is shared this way across Linux systems. On Arch/Pacman, own the Fcitx 5 daemon and McBopomofo engine natively in the Pacman/AUR profile. On non-Arch graphical systems, own the daemon and McBopomofo engine in `config/config.fcitx-flatpak.toml`; APT/DNF package fragments retain native host IM modules. Install Catppuccin and Mellow themes through the shared `pre-dotfiles` bootstrap hook for both native and Flatpak Fcitx5 instead of tracking generated theme data. Use declarative copy/symlink/template/edit modes only when deployment semantics are genuinely needed, such as the managed `.gitconfig` block.
...
- Change Fcitx5 configuration: edit `home/.config/fcitx5/profile` or `home/.config/environment.d/90-fcitx5.conf`; use `config/conf.d/packages.pacman.toml` for the native Arch daemon/engine, `config/config.fcitx-flatpak.toml` for the non-Arch Flatpak daemon/engine/autostart, APT/DNF package fragments for host IM modules, `config/scripts/sync-fcitx5-themes.sh` plus the `pre-dotfiles` hook for Catppuccin and Mellow themes, and `config/scripts/reload-fcitx5.sh` plus the `post-dotfiles` hook to activate restored configuration.
...
At minimum run syntax/static checks for changed files, `mise tasks validate --errors-only` for task/config changes, `mise bootstrap --dry-run` for bootstrap/config changes, and the relevant `MISE_ENV` selection checks for `miserc.toml`/`conf.d` changes. Do not run the full real bootstrap on the user's machine merely as a test unless explicitly requested.
